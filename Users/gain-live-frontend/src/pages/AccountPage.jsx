        ...
        <div className="relative glass-card rounded-xl p-5 overflow-hidden">
          ...
          <div className="grid grid-cols-3 gap-2 mt-4">
            <div className="bg-surface-container-lowest/50 rounded-lg p-3 text-center">
              <p className="text-lg font-headline font-black text-primary-container">à§³{(p.balance || 0).toLocaleString()}</p>
          ...
          <div className="glass-card rounded-lg overflow-hidden divide-y divide-outline-variant/10">
            ...
            <div className="px-4 py-3 flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-sm font-headline font-bold text-on-surface">{p.currency || "BDT"} (à§³)</p>
              </div>
            </div>
          </div>
        </section>
      ...
      {activeTab === "stats" && (
        <section className="space-y-4">
          ...
          <div className="grid grid-cols-2 gap-3">
            {[
              { label: "Balance", value: `à§³${(p.balance || 0).toLocaleString()}`, color: "text-primary-container", icon: "account_balance_wallet" },
              { label: "Total Deposits", value: `à§³${(p.totalDeposits || 0).toLocaleString()}`, color: "text-on-surface", icon: "south_west" },
              { label: "Total Withdrawals", value: `à§³${(p.totalWithdrawals || 0).toLocaleString()}`, color: "text-secondary-container", icon: "north_east" },
              { label: "Total Winnings", value: `à§³${(p.totalWinnings || 0).toLocaleString()}`, color: "text-tertiary-fixed-dim", icon: "emoji_events" },
            ].map((s, i) => (
          ...
          <div className="glass-card rounded-lg overflow-hidden divide-y divide-outline-variant/10">
            {[
              ...
              { label: "Referral Earnings", value: `à§³${(p.referralEarnings || 0).toLocaleString()}`, color: "text-tertiary-fixed-dim" },
            ].map((item, i) => (
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex justify-between items-center">
                      ...
                      <p className={`text-sm font-headline font-bold ${t.type === "withdraw" ? "text-secondary-container" : "text-primary-container"}`}>
                        {t.type === "withdraw" ? "-" : "+"}à§³{(t.amount || 0).toLocaleString()}
                      </p>
                    </div>
